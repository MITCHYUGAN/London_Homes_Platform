  ...
  return (
    <header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        isScrolled ? "bg-white/95 backdrop-blur-md shadow-sm border-b border-[#ECECEC] py-4" : "bg-white/80 backdrop-blur-xs py-6"
      }`}
    >
      <div className="max-w-full mx-auto px-6 lg:px-12 flex items-center justify-between">
        ...
        <nav id="desktop-nav" className="hidden lg:flex items-center space-x-8">
          {navItems.map((item) => (
            ...
            >
              {item.label}
              {currentPage === item.id && <span className="absolute bottom-0 left-0 w-full h-[1.5px] bg-[#622219] animate-fade-in" />}
            </button>
          ))}
        </nav>
        ...
        <div id="header-actions" className="hidden lg:flex items-center space-x-4">
          <a id="phone-link" href="tel:+2348032659756" className="flex items-center space-x-2 text-sm font-medium text-[#111111] hover:text-[#622219] transition-colors duration-300">
            <Phone className="w-4 h-4 text-[#622219]" />
            <span className="font-mono text-xs">+234 (0) 803 265 9756</span>
          </a>
          <button
        ...
        <div className="px-6 py-8 flex flex-col space-y-5">
          {navItems.map((item) => (
            <button
              ...
              id={`mob-nav-${item.id}`}
              onClick={() => handleNavClick(item.id)}
              className={`font-sans text-left text-base font-semibold tracking-wide py-1 focus:outline-none ${currentPage === item.id ? "text-[#622219]" : "text-[#333333]"}`}
            >
              {item.label}
            </button>
          ))}
          <div className="pt-4 border-t border-[#ECECEC] flex flex-col space-y-4">
            <a id="mob-phone-link" href="tel:+2348032659756" className="flex items-center space-x-3 text-sm font-semibold text-[#111111]">
              <Phone className="w-4 h-4 text-[#622219]" />
              <span className="font-mono text-sm">+234 (0) 803 265 9756</span>
            </a>
            <a id="mob-whatsapp-link" href="https://wa.me/2348032659756" target="_blank" rel="noreferrer" className="flex items-center space-x-3 text-sm font-semibold text-[#111111]">
